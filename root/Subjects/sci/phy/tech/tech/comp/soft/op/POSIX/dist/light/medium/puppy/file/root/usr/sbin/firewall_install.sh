...
#120508 internationalized L18L

. gettext.sh
export TEXTDOMAIN=firewall_install.sh
...
#  exit 1
#fi
[ "`whoami`" != "root" ] && exec sudo -A ${0} ${@} #120427
...
goodbye()
{
  ...
  clear
  echo "$(gettext 'Configuration terminated.  Goodbye.')"
  exit
}
...
if (( `which dialog 2>&1 | grep -c "which: no dialog in"` )); then
  echo "$(gettext 'Required program 'dialog' not found.')"
  exit 1
fi
...
while [ "$status" != "exit" ]; do
  ...
  'welcome')
#Modified by LC
   dialog --title "$(gettext 'Linux Firewall Configuration Utility')" $CANCEL_QUIT \
      --menu "`eval_gettext 'Welcome to installer version ${GUI_VERSION} for Linux Firewall version ${FW_VERSION}.  You can abort this installation at any time by pressing the [ESC] key.  Please select your desired installation type below.  The default installation will close all ports so that no services will be available to remote hosts.  A custom installation will allow you to configure remote access and provide an opportunity to enable Internet connection sharing for hosts on an attached private network.'`" 18 75 3 \
      "automagic" "$(gettext 'Default installation without questions')" \
      "default" "$(gettext 'Default installation')" \
      "custom" "$(gettext 'Custom installation')" 2> $TMPFILE
...
##
  ...
  'otherports')
    dialog $CANCEL_BACK --title "$(gettext 'Specify Open Ports')" --inputbox "$(gettext 'Numerically list additional ports or port ranges available to other connecting hosts.  In the next section you will be given a chance to specify who is allowed to connect to each port or port range.  Protocols 'tcp' and 'udp' can optionally be specified here.  If no protocol is specified, then connections to either protocol will be accepted. 
Format: <port or port-range>[/<protocol>] 
Example: ')\"901/tcp 92 2400-2500/tcp\" 
$(gettext 'This will open up TCP port 901, port 92 for both TCP and UDP, and TCP ports 2400 through 2500.')" \
    19 55 2> $TMPFILE
  ...
  'openports')
	...
	dialog $NO_CANCEL --title "`eval_gettext 'Port ${curport} of ${portnum} - ${portstr}'`" --menu "`eval_gettext 'Who can access ${portstr}?'`" 10 55 3 \
	  "any" "$(gettext 'Allow from anywhere')" \
	  "$SHORT_NETWORK_NAME" "`eval_gettext 'Allow from ${LONG_NETWORK_NAME}'`" \
	  "specify" "$(gettext 'Specify Access')" 2> $TMPFILE

      else

	dialog $NO_CANCEL --title "`eval_gettext 'Port ${curport} of ${portnum} - ${portstr}'`" --menu "`eval_gettext 'Who can access ${portstr}?'`" 9 55 2 \
	  "any" "$(gettext 'Allow from anywhere')" \
	  "specify" "$(gettext 'Specify Access')" 2> $TMPFILE
      ...
      if [ "$status" == "255" ]; then goodbye
        ...
        'specify')
          ...
          while [ "$status" == "loop" ]; do
            dialog $NO_CANCEL --title "`eval_gettext 'Specify access to ${portstr}'`" --inputbox "`eval_gettext 'Please specify which hosts and/or networks may connect to ${portstr} by entering a space-delimited list of hosts and/or networks.
...
Example: \"207.198.61.33 198.82.0.0/16\"'`" \
            ...
            if [ "$status" == "255" ]; then goodbye
              ...
              for combo in `cat $TMPFILE` ; do
                if [ "X`echo $combo | cut -s -f 1 -d '/'`" == "X" ]; then
                  ...
                  if [ "$newmask" == "-1" ]; then
                    dialog --title "$(gettext 'Invalid Bitmask')" --msgbox "`eval_gettext 'The host/netmask you specified (${host}/${mask}) was invalid.  Re-enter the full list of hosts and/or networks for port ${portstr}.'`" \
                    7 55
                    status=$?
                    if [ "$status" == "255" ]; then goodbye
                    else
  ...
  'natquestion')
    dialog --title "$(gettext 'Internet Connection Sharing')" $DEFAULT_NO --yesno "$(gettext 'Do you wish to share your Internet connection with a private internal network?')" \
    6 55

    status=$?
    if [ "$status" == "255" ]; then goodbye
    elif [ "$status" == "0" ]; then
      dialog $CANCEL_BACK --title "$(gettext 'List Internal Interfaces')" --inputbox "$(gettext 'Enter one or more ethernet interfaces connected to private internal networks in a space delimited list.
Example: "eth1 eth2"')" \
      9 55 2> $TMPFILE
      status=$?
	...
	if [ -n "$int_iface_opt" ]; then
	  dialog --title "$(gettext 'Dial-up Information')" $DEFAULT_NO --yesno "$(gettext 'Does this system access the Internet through a telephone dial-up modem?  This information is required in order to configure Internet connection sharing and will not effect your Internet access in any way.')" \
	  9 55
  ...
  'trustednet')
    if [ "X$int_iface_opt" == "X" ]; then
      ...
      xtratxt="$(gettext ' (besides existing private internal networks)')"
      height="15"
    fi
    status="loop"
    while [ "$status" == "loop" ]; do
      dialog --title "$(gettext 'Trusted Networks')" --inputbox "`eval_gettext 'If there are any hosts or networks${xtratxt} that should be able to bypass the firewall altogether and connect to any services running on this system, please list them here.  This might include hosts allowed to connect here for administrative purposes. 
Format: <host or network address>[/<netmask>] 
Example: \"207.198.61.33 128.173.0.0/16\"'`" \
	...
	for combo in `cat $TMPFILE` ; do
	  if [ "X`echo $combo | cut -s -f 1 -d '/'`" == "X" ]; then
	    ...
	    newmask=`make_mask $mask`
	    if [ "$newmask" == "-1" ]; then
	      dialog --title "$(gettext 'Invalid Bitmask')" --msgbox "`eval_gettext 'The host/netmask you specified (${host}/${mask}) was invalid.  Re-enter the complete full-access list.'`" \
	      7 55
	      status=$?
	      if [ "$status" == "255" ]; then goodbye
		status="loop"
...
#Modified by LC
  if [ "$AUTO" == "on" ]; then
    ...
    if [ "$status" == "255" ]; then goodbye
      ...
      clear
      echo "$(gettext 'Running './rc.firewall check'.  Output will follow ...')"
      echo
	...
	#echo
	echo "
$(gettext 'Errors were detected in your system configuration.
See the output above for specific details.')

`eval_gettext 'A copy of the Linux Firewall initialization script preconfigured by this program is located in ${FW_TMPFILE}
'`"	
	rm -f $TMPFILE > /dev/null 2>&1
	exit 1
      fi
...
#Modified by LC
      if [ "$AUTO" == "on" ]; then
         ...
         echo -n "$(gettext 'Press any key to continue ... ')"
         read -rsn1
      fi
    fi
    status="install"
...
#Modified by LC
  if [ "$AUTO" == "on" ]; then
     echo "$(gettext 'Firewall installation...')"
     status=0
  else
     dialog --title "$(gettext 'Firewall Installation')" --yesno "`eval_gettext 'The firewall is now ready to be enabled.  Your system configuration will also be modified so that the firewall will be started each time your computer is booted.  If you choose not to continue, a copy of the Linux Firewall initialization script, preconfigured by this program, can be found here: \"${FW_TMPFILE}\".  Would you like to continue?'`" \
    12 55
  fi    
    status=$?
    if [ "$status" == "255" ]; then goodbye
      ...
      if [ "$status" != "0" ]; then
	clear
	echo "$(gettext 'INSTALLATION FAILED with the following message:')"
	echo
	echo "\"Unable to write file to $FW_INSTALL.\""
	echo "`eval_gettext 'A copy of the Linux Firewall initialization script preconfigured by this program is located in ${FW_TMPFILE}.'`"
	echo
	rm -f $TMPFILE > /dev/null 2>&1
	exit 1
      fi
...
EOF
	...
	echo
	echo "`eval_gettext 'The firewall is now RUNNING on your system.  
The firewall initialization script has been installed here: ${FW_INSTALL} 
and will run each time you boot your system.'`"
	echo "$(gettext 'Advanced users can modify their firewall configuration by configuring the above file with any text editor. After making changes you will need to run the rc.firewall script again for changes to take effect.')"
	echo
	echo "$(gettext 'For more information, please visit:   http://projectfiles.com/firewall/')"
	rm -f $TMPFILE > /dev/null 2>&1
	exit
      else
	clear
